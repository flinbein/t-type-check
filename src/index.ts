type AssertDataFn<T> = <const A>(
	data: unknown extends A ? A : T extends A ? A : T,
	errorMessage?: string
) => unknown extends A ? T : A & T

export type ResolveTypeChecker<T extends CheckTypeFn> = T extends CheckTypeFn<infer R> ? R : never;

type CheckTypeFn<T = any> = {
	(arg: unknown): arg is T
}
type CheckTypeAssertFn<T = any> = CheckTypeFn<T> & {
	assert: AssertDataFn<T>
};
type CheckTypeWithHelpersFn<T = any> = CheckTypeAssertFn<T> & {
	optional: CheckTypeAssertFn<T | undefined>
};
type CheckTypeStringWithHelpersFn<T = any> = CheckTypeWithHelpersFn<T> & {
	startsWith<const T extends readonly string[]>(...tokens: T): CheckTypeWithHelpersFn<`${T[number]}${string}`>
	endsWith<const T extends readonly string[]>(...tokens: T): CheckTypeWithHelpersFn<`${string}${T[number]}`>
	includes<const T extends readonly string[]>(...tokens: T): CheckTypeWithHelpersFn<`${string}${T[number]}${string}`>
};
type TypeChecker =
	| CheckTypeFn
	| readonly TypeChecker[]
	| {[key: string | number | symbol]: TypeChecker}
	| null | boolean | number | string | undefined | RegExp
	;

type TypeExtract<T extends TypeChecker> =
	T extends CheckTypeFn<infer KT> ? KT
...
type TypeCheck = {
	...
	readonly string: CheckTypeStringWithHelpersFn<string>
	...
	readonly keyValueOf: <const K extends TypeChecker, const V extends TypeChecker>(
		keyType:
			K extends string ? K
				: K extends (arg: any) => arg is infer KT ?
					[KT] extends [string] ? K : CheckTypeFn<string>
					: CheckTypeFn<string>,
		valueType: V
	) =>
		[TypeExtract<K>, TypeExtract<V>] extends [infer KEYS extends keyof any, infer VALUES]
			? CheckTypeWithHelpersFn<Partial<Record<KEYS, VALUES>>>
			: CheckTypeWithHelpersFn<never>;
...
function withHelpers<T extends (args: any) => boolean>(fn: T, customHelpers?: Record<any, any>): T {
	...
	if (customHelpers) {
		for (const key in customHelpers) (fn as any)[key] = customHelpers[key];
	}
	return fn as any;
}
...
const stringStartsWith = (...tokens: string[]) => withHelpers((value: unknown) => typeof value === "string" && tokens.some(t => value.startsWith(t)));
const stringEndsWith = (...tokens: string[]) => withHelpers((value: unknown) => typeof value === "string" && tokens.some(t => value.endsWith(t)));
const stringIncludes = (...tokens: string[]) => withHelpers((value: unknown) => typeof value === "string" && tokens.some(t => value.includes(t)));
...
T.keyValueOf = (keyType: TypeChecker, valueType: TypeChecker) => {
	return withHelpers((value: unknown) => {
		if (!value) return false;
		if (!(typeof value === "object")) return false;
		if (Array.isArray(value)) return false;
		for (let [key, item] of Object.entries(value)) {
			if (!checkType(keyType, key)) return false;
			if (!checkType(valueType, item)) return false;
		}
		return true;
	})
}
