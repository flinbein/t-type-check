...
test("T keyValueOf", () => {
	const isCorrectItem = T.keyValueOf(T.string.startsWith("a"), T.bool);
	
	expect(isCorrectItem({})).toBe(true);
	expect(isCorrectItem({a: true})).toBe(true);
	expect(isCorrectItem({a: true, a1: true, a2: false})).toBe(true);
	expect(isCorrectItem({a1: true, a2: "text"})).toBe(false);
	expect(isCorrectItem({a1: true, zz: true})).toBe(false);
	expect(isCorrectItem([])).toBe(false);
	
	const expectedError =new Error("wrong data format");
	
	expect(() => isCorrectItem.assert({a1: true, zz: true} as any)).toThrow(expectedError);
	expect(() => isCorrectItem.assert([] as unknown)).toThrow(expectedError);
});
...
test("T assertions", () => {
	...
	expect(T.objectPartOf({x: T.number}, T.bool).assert(xyMap as unknown)).toBe(xyMap);
...
test("T negative assertions", () => {
	...
	expect(() => T.string.assert(4 as unknown)).toThrow(expectedError);
	expect(() => T.number.assert("x" as unknown)).toThrow(expectedError);
	expect(() => T.int.assert(true as unknown)).toThrow(expectedError);
	expect(() => T.bool.assert(3 as unknown)).toThrow(expectedError);
	expect(() => T.instanceOf(Date).assert(12 as unknown)).toThrow(expectedError);
	expect(() => T.optionalOf(2).assert(3 as unknown)).toThrow(expectedError);
	expect(() => T.allOf(T.number, T.int).assert(12.2)).toThrow(expectedError);
	expect(() => T.oneOf(T.number, T.string).assert(false as unknown)).toThrow(expectedError);
	expect(() => T.arrayOf(T.int).assert([1,2,3,4.4])).toThrow(expectedError);
	expect(() => T.mapOf(T.number, T.bool).assert({x: 1, y: true, z: "er"} as unknown)).toThrow(expectedError);
	expect(() => T.objectPartOf({x: T.number}, T.bool).assert({x: 1, y: 2} as unknown)).toThrow(expectedError);
	expect(() => T.listPartOf([1], T.number).assert([2,3] as unknown)).toThrow(expectedError);
	
	expect(() => T.string.assert(4 as unknown, "fooBar")).toThrow(new Error("fooBar"));
});
